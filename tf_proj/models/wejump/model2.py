...
def _inference(options, features, is_training):
    ...
    with tf.variable_scope('conv4'):
        out = utils.make_conv_bn_relu(out, [3, 3, 16, 64], 1, is_training)
        out = tf.nn.max_pool(out, [1, 2, 2, 1], [1, 2, 2, 1], padding='SAME')
        #out_tmp = tf.transpose(out, perm=[3, 1, 2, 0])[:, :, :, 0:1]
        #tf.summary.image('conv4', out_tmp)

    with tf.variable_scope('conv5'):
        out = utils.make_conv_bn_relu(out, [2, 2, 64, 128], 1, is_training)
        out = tf.nn.max_pool(out, [1, 2, 2, 1], [1, 2, 2, 1], padding='SAME')
        #out_tmp = tf.transpose(out, perm=[3, 1, 2, 0])[:, :, :, 0:1]
        #tf.summary.image('conv5', out_tmp)
        
    out = tf.reshape(out, [-1, 128 * 80 * 90])
    with tf.variable_scope('fc6'):
        out = utils.make_fc(out, [128 * 80 * 90, 128], keep_prob)

    with tf.variable_scope('fc7'):
        out = utils.make_fc(out, [128, 2], keep_prob)
    return out
